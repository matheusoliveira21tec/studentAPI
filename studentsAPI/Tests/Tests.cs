...
namespace studentsAPI.Tests
{
    public class Tests
    {
        ...
        public Tests()
        {
            studentService = new StudentController(new Mock<DataContext>().Object);
        }

        #region ValidatingSendingID

        [Fact]
        public async Task Post_SendingId()
        {
            var exception = await Assert.ThrowsAsync<Exception>(() => studentService.Post(new Student { Id = 1 }));
